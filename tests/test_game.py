...
def test_native_engine() -> bool:
    """The ROM-free path must search legally without importing a ROM."""
    from pyqiwang import NativeQiWangEngine

    engine = NativeQiWangEngine(depth=1)
    board = Board()
    before = (list(board.cells), [list(x) for x in board.pieces],
              board.side_to_move, list(board.move_history))
    move = engine.get_best_move(board)
    after = (list(board.cells), [list(x) for x in board.pieces],
             board.side_to_move, list(board.move_history))
    legal = generate_legal_moves(board, board.side_to_move)
    analysis = engine.analyze(board)
    ok = (move in legal and before == after
          and analysis['backend'] == 'native'
          and analysis['faithful'] is False
          and analysis['nodes'] > 0)
    print(f"native engine ROM-free legal search: {move} "
          f"{'ok' if ok else 'WRONG'}")
    return ok


def test_native_opening_book() -> bool:
    """The extracted native book must reproduce all 33 legal ROM plies."""
    from pyqiwang import NativeQiWangEngine
    from pyqiwang._book import OPENING_LINE

    engine = NativeQiWangEngine(depth=1, book=True)
    board = Board()
    matched = 0
    for expected in OPENING_LINE:
        move = engine.get_best_move(board)
        if move != expected or move not in generate_legal_moves(board, board.side_to_move):
            print(f"  ! native book ply {matched}: expected={expected} got={move}")
            return False
        info = engine.analyze(board)
        if not info['from_book'] or info['nodes'] != 0:
            print(f"  ! native book analysis at ply {matched}: {info}")
            return False
        board.make_move(*move)
        matched += 1

    # Once the line is exhausted, get_best_move must fall through to search.
    info = engine.analyze(board)
    ok = matched == 33 and not info['from_book'] and info['nodes'] > 0
    print(f"native opening book: {matched}/33 then search "
          f"{'ok' if ok else 'WRONG'}")
    return ok


def test_native_book_divergence() -> bool:
    """A position off the fixed line must not receive a book move."""
    from pyqiwang import NativeQiWangEngine
    from pyqiwang._book import OPENING_LINE

    engine = NativeQiWangEngine(depth=1, book=True)
    board = Board()
    book_first = OPENING_LINE[0]
    alternative = next(move for move in generate_legal_moves(board, board.side_to_move)
                       if move != book_first)
    board.make_move(*alternative)
    info = engine.analyze(board)
    ok = not info['from_book'] and info['nodes'] > 0
    print(f"native book divergence falls back to search: {'ok' if ok else 'WRONG'}")
    return ok


def test_golden_fixture() -> bool:
    """The checked-in ROM baseline must be well-formed and self-consistent."""
    path = Path(__file__).with_name('fixtures') / 'rom_depth1_golden.json'
    data = json.loads(path.read_text(encoding='utf-8'))
    cases = data.get('cases', [])
    ok = (data.get('format') == 1
          and data.get('source', {}).get('depth') == 1
          and len(cases) == 12)
    from pyqiwang import NativeQiWangEngine
    native = NativeQiWangEngine(depth=1)
    agreement = 0
    for case in cases:
        board = Board()
        board.pieces[RED] = list(case['board']['red'])
        board.pieces[BLACK] = list(case['board']['black'])
        board.side_to_move = case['board']['side']
        board.move_history = []
        board._init_board()
        move = tuple(case['best_move']) if case['best_move'] is not None else None
        if evaluate_raw(board) != case['evaluation_raw']:
            ok = False
        if move is not None and move not in generate_legal_moves(board, board.side_to_move):
            ok = False
        agreement += native.get_best_move(board) == move
    # Depth-1 parity is a hard regression condition for this deterministic,
    # deliberately small corpus. It does not establish general ROM fidelity.
    ok = ok and agreement == len(cases)
    print(f"ROM depth-1 golden fixture: {len(cases)}/12 valid, "
          f"native agreement {agreement}/12 {'ok' if ok else 'WRONG'}")
    return ok


def test_native_quiescence_horizon_check_score() -> bool:
    """A horizon check is scored normally until a king is captured."""
    from pyqiwang import NativeQiWangEngine
    from pyqiwang._native import INFINITY, MATE_SCORE

    path = Path(__file__).with_name('fixtures') / 'rom_depth2_independent.json'
    data = json.loads(path.read_text(encoding='utf-8'))
    case = next(item for item in data['cases'] if item['id'] == 'random-004')
    board = Board()
    board.pieces[RED] = list(case['board']['red'])
    board.pieces[BLACK] = list(case['board']['black'])
    board.side_to_move = case['board']['side']
    board.move_history = []
    board._init_board()
    board.make_move(100, 101)
    board.make_move(62, 50)

    engine = NativeQiWangEngine(depth=2, book=False)
    score = engine._quiesce(board, RED, -INFINITY, INFINITY, 2)
    ok = is_in_check(board, RED) and abs(score) < MATE_SCORE
    print(f"native horizon check keeps PST score: score={score} "
          f"{'ok' if ok else 'WRONG'}")
    return ok


def test_native_depth8_bounded() -> bool:
    """Depth 8 must be addressable and safely return a completed iteration."""
    from pyqiwang import NativeQiWangEngine

    engine = NativeQiWangEngine(depth=8, node_limit=900)
    board = Board()
    before = (list(board.cells), [list(x) for x in board.pieces],
              board.side_to_move, list(board.move_history))
    info = engine.analyze(board)
    after = (list(board.cells), [list(x) for x in board.pieces],
             board.side_to_move, list(board.move_history))
    ok = (info['depth'] == 8 and info['stopped']
          and info['completed_depth'] >= 1
          and info['move'] in generate_legal_moves(board, board.side_to_move)
          and info['pv'] and info['pv'][0] == info['move']
          and before == after)
    print(f"native bounded depth-8 skeleton: completed={info['completed_depth']} "
          f"nodes={info['nodes']} {'ok' if ok else 'WRONG'}")
    return ok


def _load_fixture_board(case: dict) -> Board:
    board = Board()
    board.pieces[RED] = list(case['board']['red'])
    board.pieces[BLACK] = list(case['board']['black'])
    board.side_to_move = case['board']['side']
...
def test_independent_depth1_fixture() -> bool:
    """Pin current parity on the independent depth-1 corpus."""
    from pyqiwang import NativeQiWangEngine

    path = Path(__file__).with_name('fixtures') / 'rom_depth1_independent.json'
    data = json.loads(path.read_text(encoding='utf-8'))
    engine = NativeQiWangEngine(depth=1)
    agreement = 0
    for case in data['cases']:
        board = _load_fixture_board(case)
        want = tuple(case['best_move']) if case['best_move'] else None
        agreement += engine.get_best_move(board) == want
    ok = len(data['cases']) == 24 and agreement == 23
    print(f"independent ROM depth-1: {agreement}/24 "
          f"{'ok' if ok else 'WRONG'}")
    return ok


def test_native_make_move() -> bool:
    """Native make_move must reject illegal moves and update legal ones."""
    from pyqiwang import NativeQiWangEngine
    ...
    move = engine.get_best_move(board)
    if move is None:
        print("native make_move: no move")
        return False
    before_side = board.side_to_move
    legal_ok = engine.make_move(board, *move)
    illegal_ok = engine.make_move(board, -1, -1)
    ok = legal_ok and not illegal_ok and board.side_to_move != before_side
    print(f"native make_move validation: {'ok' if ok else 'WRONG'}")
    return ok
...
def main() -> int:
    ...
    results = [
        ...
        test_native_engine(),
        test_native_opening_book(),
        test_native_book_divergence(),
        test_golden_fixture(),
        test_independent_depth1_fixture(),
        test_native_quiescence_horizon_check_score(),
        test_native_depth8_bounded(),
        test_native_make_move(),
    ]
    if a.slow:
